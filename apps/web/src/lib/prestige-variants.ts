...
<header class="bg-surface/95 dark:bg-on-background/95 backdrop-blur-md sticky top-0 w-full z-50 border-b border-outline-variant/30 dark:border-outline/20 transition-all duration-300 ease-in-out">
  <nav class="flex justify-between items-center max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop h-20">
    ...
    <button class="hidden md:inline-flex items-center justify-center bg-primary text-on-primary hover:bg-primary/90 transition-colors px-6 py-3 rounded-lg font-label-sm text-label-sm uppercase tracking-wider">
      Consultoría
    </button>
    <button class="md:hidden text-primary p-2" onclick="this.closest('header, nav').querySelector('[data-mobile-menu]').classList.toggle('hidden')" aria-label="Abrir menú">
      <span class="material-symbols-outlined">menu</span>
    </button>
  </nav>
  <div data-mobile-menu class="hidden md:hidden flex flex-col gap-4 bg-surface dark:bg-on-background border-t border-outline-variant/20 px-margin-mobile py-6">
    ${(c.links || []).map((l: any) => `<a class="text-on-surface-variant dark:text-outline-variant hover:text-primary dark:hover:text-primary-fixed transition-colors py-2 font-body-md text-body-md" href="${l.url}">${l.label}</a>`).join("")}
    <button class="inline-flex items-center justify-center bg-primary text-on-primary hover:bg-primary/90 transition-colors px-6 py-3 rounded-lg font-label-sm text-label-sm uppercase tracking-wider mt-2">Consultoría</button>
  </div>
</header>`;
    }
  ...
  <div class="relative z-10 w-full max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-section-padding">
    <div class="max-w-2xl">
      ...
      <div class="flex flex-col sm:flex-row gap-4">
        ${c.buttonText ? `<button class="bg-primary text-on-primary hover:bg-primary/90 transition-colors px-8 py-4 rounded-lg font-label-sm text-label-sm uppercase tracking-wider flex items-center justify-center gap-2">
          ${c.buttonText}
          <span class="material-symbols-outlined text-[18px]">arrow_forward</span>
        </button>` : ""}
        <button class="border border-outline-variant text-on-background hover:bg-surface-container-low transition-colors px-8 py-4 rounded-lg font-label-sm text-label-sm uppercase tracking-wider">
  ...
  <div class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-gutter">
      ...
      <div class="bg-surface border border-outline-variant/30 rounded-xl p-8 hover:shadow-[0_4px_12px_rgba(15,23,42,0.04)] transition-all duration-300 flex flex-col h-full group">
        ...
        <a class="inline-flex items-center font-label-sm text-label-sm text-primary uppercase tracking-wider group-hover:underline underline-offset-4" href="#">Conocer más <span class="material-symbols-outlined text-[16px] ml-1">arrow_right_alt</span></a>
      </div>`).join("")}
    </div>
  </div>
</section>`;
