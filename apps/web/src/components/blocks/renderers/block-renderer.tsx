...
export function BlockRenderer({ type, content }: { type: string; content: any }) {
  ...
  switch (type) {
    ...
    case "linktree": {
      ...
      return (
        <div style={bgStyle} className="min-h-screen w-full flex flex-col items-center py-16 px-4 relative">
          {linktree.background?.type === "image" && (
          ...
          <div className="w-full max-w-sm mx-auto relative z-10 animate-[fadeUp_.6s_cubic-bezier(.16,1,.3,1)_both]">
            ...
            <div className="flex flex-col items-center text-center mb-10">
              {linktree.logoUrl ? (
                <img src={resolveMediaUrl(linktree.logoUrl)} alt={linktree.title}
                  className="max-h-[100px] w-auto max-w-[200px] object-contain mb-6"
            ...
            {linktree.socials?.length > 0 && (
              <div className="flex flex-wrap justify-center gap-3 mb-10">
                {linktree.socials.map((s: any, idx: number) => {
                  ...
                  const icon = socialIcons[s.platform] || <span className="text-xs font-bold">{s.platform?.substring(0, 2)}</span>;
                  return (
                    <a key={idx} href={s.url} target="_blank" rel="noopener noreferrer"
                      className="w-12 h-12 rounded-full flex items-center justify-center text-white transition-all duration-300 hover:-translate-y-1 hover:scale-110 shadow-lg"
            ...
            <div className="mt-14 text-center">
              <a href="https://icebergup.com" target="_blank" rel="noopener noreferrer"
